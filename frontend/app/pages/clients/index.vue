  ...
  <div class="flex-1 overflow-y-auto">
    <div class="p-6 space-y-4">
      ...
      <UCard :ui="{ body: 'p-0' }">
        ...
        >
          <template #name-cell="{ row }">
            <NuxtLink :to="`/clients/${row.original.id}`" class="font-medium hover:text-primary-500">
              {{ row.original.name }}
            </NuxtLink>
          </template>
          <template #isBlacklisted-cell="{ row }">
            <UBadge v-if="row.original.isBlacklisted" color="error" variant="soft" size="sm">블랙리스트</UBadge>
          </template>
          <template #platform-cell="{ row }">
            <UBadge variant="outline" size="sm">{{ row.original.platform }}</UBadge>
          </template>
      ...
      <div class="flex justify-center">
        <UPagination v-model:page="page" :total="total" :items-per-page="20" @update:page="fetchClients" />
      </div>
    </div>
  </div>
</template>
